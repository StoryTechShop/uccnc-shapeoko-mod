...
/**
 * StoryTechShop Modifications
 * - 
 */

description = "UCCNC StoryTechShop";
vendor = "UCCNC";
...
longDescription = "Generic post for machines using UCCNC control software. If you are using a fourth axis, make sure to select which axis it is mounted along and the feedrate type (Inverse time or the default, degrees per minute). If you have an automatic tool changer you need to enable the property 'useToolChanger'. For laser cutting, set your desired power setting by using property 'Laser power percentage'.";
...
var probeWCSFormat = createFormat({prefix:"W", decimals:0, forceDecimal:true});
...
function onPower(power) {
  var cuttingPower = 255 * (getProperty("laserPowerPrecentage") / 100);
...
function onCyclePoint(x, y, z) {
  ...
  if (isProbeOperation()) {
    ...
    if (printProbeResults()) {
      // TODO: Print Probe Results not currently supported
  ...
  if (forceCycle || isFirstCyclePoint() || isProbeOperation()) {
    ...
    case "probing-xy-circular-partial-boss":
      ...
      writeBlock(
        mFormat.format(32), "{",
        "P" + 4, // bore/boss 3-point probe routine
    ...
    case "probing-xy-inner-corner":
      ...
      if ((cornerI != 0) && (cornerJ != 0)) {
        if (currentSection.strategy == "probe") {
          setProbeAngleMethod();
          probeVariables.compensationXY = "X[#2100] Y[#2101]";
        }
      }
      protectedProbeMove(cycle, x, y, z - cycle.depth);
      writeBlock(
    ...
    case "probing-x-plane-angle":
      ...
      writeBlock(
        mFormat.format(32), "{",
        "P" + 7, // surface angle probe routine
        "X" + xyzFormat.format(x + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2)),
...
function protectedProbeMove(_cycle, x, y, z) {
  ...
  if (_z && z >= getCurrentPosition().z) {
    writeBlock(mFormat.format(213)); // protected probe positioning move
    writeBlock(gFormat.format(01), _z, getFeed(cycle.feedrate));
    writeBlock(mFormat.format(214));
  }
  if (_x || _y) {
    writeBlock(mFormat.format(213)); // protected probe positioning move
    writeBlock(gFormat.format(01), _x, _y, getFeed(highFeedrate));
    writeBlock(mFormat.format(214));
  }
  if (_z && z < getCurrentPosition().z) {
    writeBlock(mFormat.format(213)); // protected probe positioning move
...
function getProbingArguments(cycle, updateWCS) {
  ...
  if (outputWCSCode) {
    validate(
      probeOutputWorkOffset > 0 && probeOutputWorkOffset < 7,
  ...
  return [
    ...
    ((cycle.updateToolWear && cycleType !== "probing-z") ? "H" + xyzFormat.format(cycle.toolDiameterOffset) : undefined),
    (cycle.updateToolWear ? "V" + xyzFormat.format(cycle.toolWearUpdateThreshold ? cycle.toolWearUpdateThreshold : 0) : undefined),
    (cycle.printResults ? "O" + xyzFormat.format(1 + cycle.incrementComponent) : undefined), // 1 for advance feature, 2 for reset feature count and advance component number. first reported result in a program should use W2.
    conditional(outputWCSCode, probeWCSFormat.format(probeOutputWorkOffset))
  ];
}
...
//var useInverseTimeFeed = true; // use 1/T feeds. added as a post property 'getProperty("useInverseTime'")
...
function onRewindMachine(_a, _b, _c) {
  //needed if rotary axis have limits
  //refer to the onRewindMachine section in CAM Post Processor Guide
}
